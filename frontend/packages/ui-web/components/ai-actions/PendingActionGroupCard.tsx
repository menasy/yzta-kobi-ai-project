...
interface PendingActionGroupCardProps {
  pendingActionGroup: AiPendingActionGroup;
  onActionMessage?: (content: string) => Promise<unknown>;
  className?: string;
}

function getExpiryLabel(expiresAt?: string | null): string {
  if (!expiresAt) return "Sınırlı süre geçerli";
  const remainingMs = new Date(expiresAt).getTime() - Date.now();
  if (Number.isNaN(remainingMs)) return "Sınırlı süre geçerli";
  ...
  if (tailMinutes === 0) return `${remainingHours} sa kaldı`;
...
export function PendingActionGroupCard({
  ...
  const canSubmit =
    ...
    isAdmin &&
    pendingActionGroup.requiresConfirmation &&
    pendingActionGroup.status === "pending";
    ...
    >
      <CardHeader className="gap-3 pb-4">
        <div className="flex flex-wrap items-center gap-2">
          <PendingActionSafetyBadge safetyLevel={pendingActionGroup.safetyLevel} />
          <Badge variant="outline" className="px-2.5 py-1 text-[11px] bg-indigo-50/50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800">
            {pendingActionGroup.actionCount} İşlem Grubu
          </Badge>
          <Badge variant="outline" className="px-2.5 py-1 text-[11px]">
            {getExpiryLabel(pendingActionGroup.expiresAt)}
          </Badge>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <PendingActionConfirmControls
          onConfirm={() => confirmAction(pendingActionGroup.groupId)}
          onCancel={() => cancelAction(pendingActionGroup.groupId)}
          isPending={isPending}
